# EAGLE optimization using ITREX
...
Intel Extension for Transformers supports EAGLE to speedup inference by up to 3x.

Unlike Assisted Generation or Speculative Sampling, EAGLE does not require a smaller sized draft version of the target model. Instead, EAGLE uses fine-tuned extra layers in the target model to sample in feature space (not embedded space) multiple tokens simultaneously. It does leverage the acceptance/rejection methodology of speculative sampling to produce a fast method of token generation that matches the quality of the target model.

See the [paper](https://arxiv.org/pdf/2401.15077.pdf) or the [blog](https://sites.google.com/view/eagle-llm) or the [repo](https://github.com/SafeAILab/EAGLE) for more information on EAGLE.


## Contents

- [EAGLE optimization using ITREX](#eagle-optimization-using-itrex)
  - [](#)
  - [Contents](#contents)
